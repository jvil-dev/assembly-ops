//
//  VideoEquipmentListView.swift
//  AssemblyOps
//
//  Created by Jorge Villeda on 2/27/26.
//

// MARK: - Video Equipment List View
//
// Filterable list of video equipment items. Category filter shows only
// video-relevant categories. Supports search, create, and bulk-create.
// Tap item → VideoEquipmentDetailView.

import SwiftUI

struct VideoEquipmentListView: View {
    @StateObject private var viewModel = VideoEquipmentViewModel()
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: AppTheme.Spacing.xl) {
                // Category filter — video categories only
                categoryFilterRow
                ...
                if viewModel.isLoading {
                    ...
                    LazyVStack(spacing: AppTheme.Spacing.m) {
                        ForEach(Array(viewModel.filteredEquipment.enumerated()), id: \.element.id) { index, item in
                            NavigationLink(destination: VideoEquipmentDetailView(equipmentId: item.id)) {
                                equipmentRow(item)
                            }
                            .buttonStyle(.plain)
        ...
        .sheet(isPresented: $showCreateSheet) {
            CreateVideoEquipmentSheet(viewModel: viewModel)
        }
        .sheet(isPresented: $showBulkCreateSheet) {
            BulkCreateVideoEquipmentSheet(viewModel: viewModel)
        }
    ...
    // MARK: - Category Filter (Video categories only)
    ...
    private var categoryFilterRow: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: AppTheme.Spacing.s) {
                ...
                ForEach(Array(AudioEquipmentCategoryItem.videoRelevantCategories).sorted(by: { $0.displayName < $1.displayName }), id: \.self) { category in
