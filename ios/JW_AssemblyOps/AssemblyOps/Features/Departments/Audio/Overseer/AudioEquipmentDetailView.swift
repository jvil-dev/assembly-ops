    ...
    var body: some View {
        Group {
            if isLoading {
                ...
                ContentUnavailableView("av.equipment.notFound".localized, systemImage: "shippingbox")
                    .themedBackground(scheme: colorScheme)
            }
        }
        ...
        .sheet(isPresented: $showReportDamage) {
            ReportDamageView(equipmentId: equipmentId, equipmentName: itemData?.name ?? "")
        }
...
#Preview {
    NavigationStack {
        AudioEquipmentDetailView(equipmentId: "preview-id")
    }
}
