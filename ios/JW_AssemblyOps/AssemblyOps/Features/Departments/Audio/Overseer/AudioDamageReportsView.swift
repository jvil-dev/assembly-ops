//
//  AudioDamageReportsView.swift
//  AssemblyOps
//
//  Created by Jorge Villeda on 2/27/26.
//

// MARK: - AV Damage Reports View
//
// List of damage reports with resolved/unresolved toggle.
// Severity badges on each row. Tap to resolve.

import SwiftUI

struct AudioDamageReportsView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showResolve) {
            if let report = selectedReport {
                ResolveDamageSheet(report: report, viewModel: viewModel)
            }
        }
    ...
    private func damageRow(_ report: AudioDamageReportItem) -> some View {
        Button {
            ...
            VStack(alignment: .leading, spacing: AppTheme.Spacing.s) {
                HStack {
                    Image(systemName: report.equipmentCategory?.icon ?? "shippingbox")
